...
## LATEST DELTA — inspected specialist bridge envelope adapter
...
## LATEST DELTA — durable three-app staged end-to-end and identity-provider boundary
- Previous `437877ba` CI #36253277089 SUCCESS. Added `pwa-central-durable-e2e.test.js`, wired into Enforcement CI: real staged browser pipeline -> scoped outbox -> central HTTP endpoint using fixture bearer/family verification -> LocalJsonStrongStore -> exact observation ACK. It covers duplicate, missing skill HOLD, member switch, rejected token and unchanged durable record. Expanded to original Hide and Snap bridge envelopes plus explicit Ready observation; all three remain observation-only without verifier receipt. This is fixture-integrated, not live Google, mobile browser, or deployed service.
- Hardened specialist mapper: conflicting nested `payload.member_id`, `payload.child_id` or `payload.family_id` cannot be hidden by a matching top-level event or client session; regression tests added.
- Product auth cross-check: Ready-Set main `/api/auth/session` currently maps Netlify Identity into a family session (`source:NETLIFY_IDENTITY`), while this central host verifies signed Google OIDC ID tokens and independently checks a server registry. Neither a Ready browser session object nor a Netlify Identity token can be silently treated as a verified central Google Bearer credential. Actual product binding stays OPEN until an explicit approved Google session/registry or a separately reviewed federation/exchange contract exists; no fake authentication, no raw child_id as authority.
- OPEN: credentialed Google auth/session and explicit selected child, browser bundle distribution and actual product opt-in, real IndexedDB multi-tab/device test, central hosting. No product main, TAKY main or Netlify mutation.
