...
/**
 * Family praise is authored by a family member with explicit server-verified gift permission, NOT child achievement evidence.
 ...
 */
const VERSION='TAKY_FAMILY_PRAISE_GIFT_V2';
...
function createFamilyPraiseGiftService({
  resolveServerSession,verifyRequestOrigin,lookupIdentityMember,authorizeFamilyGiver,
  listGiftableBadges,resolveGiftableBadge,appendGiftRecord
}={}){
  if([resolveServerSession,verifyRequestOrigin,lookupIdentityMember,authorizeFamilyGiver,listGiftableBadges,
    resolveGiftableBadge,appendGiftRecord].some(fn=>typeof fn!=='function'))
    throw Error('TRUSTED_FAMILY_GIFT_CAPABILITIES_REQUIRED');

  async function giverSession(request,mutation=false,targetChildId=null){
    if(mutation){
    ...
    if(!session||session.authenticated!==true||session.source!=='NETLIFY_IDENTITY'||
      ...
      return fail('VERIFIED_FAMILY_SESSION_REQUIRED');
    let member;
    try{member=await lookupIdentityMember(session.member_id)}
    catch{return fail('GIVER_MEMBER_LOOKUP_FAILED')}
    if(!member||member.identity_verified!==true||member.member_id!==session.member_id||
      !clean(member.family_id)||member.family_id!==session.family_id)
      return fail('VERIFIED_GIVER_FAMILY_MEMBERSHIP_REQUIRED');
    // A family relationship alone never grants the ability to transfer gifts.
    // A trusted family permission provider must explicitly authorize the capability.
    let grant;
    try{grant=await authorizeFamilyGiver({
      family_id:session.family_id,giver_member_id:session.member_id,
      permission:'FAMILY_PRAISE_GIFT',
      action:mutation?'SEND_GIFT':'VIEW_GIFT_OPTIONS',
  ...
  async function getGiftOptions(request){
    const auth=await giverSession(request);
    ...
    // This giver-only view is not a personal badge collection for any family member.
    return {ok:true,contract:VERSION,gem_amount_options:[...GEM_OPTIONS],badge_options};
  }

  async function sendGift(request,input={}){
    const auth=await giverSession(request,true,clean(input?.target_child_id));
    if(!auth.ok)return auth;
    if(!input||typeof input!=='object'||Array.isArray(input))return fail('INVALID_GIFT_REQUEST');
    if(['family_id','giver_parent_id','giver_member_id','giver_role','family_relation','gift_permission','gift_id','award_id','award_kind','decision_status',
    ...
    if(input.explicit_gift_action!==true)return fail('EXPLICIT_GIFT_ACTION_REQUIRED');
    ...
    if(auth.session.member_id===target)return fail('SELF_GIFT_FORBIDDEN');
    const record={
      contract:VERSION,source:'FAMILY_PRAISE',
      family_id:auth.session.family_id,
      giver_member_id:auth.session.member_id,receiver_child_id:target,
...
module.exports=Object.freeze({VERSION,MAX_GEMS_PER_GIFT,GEM_OPTIONS,createFamilyPraiseGiftService});
