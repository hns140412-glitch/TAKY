...
// Must be invoked AFTER server-origin, authorized family giver, target-child membership
// and approved family praise badge checks in family-praise-gift.js.
// This is a GIFT JOURNAL, not Achievement Award Ledger or child gem balance.
const fs=require('node:fs');
const path=require('node:path');
const crypto=require('node:crypto');
const CONTRACT='TAKY_FAMILY_PRAISE_GIFT_JOURNAL_V2';
...
function createFamilyGiftJournal({directory,family_id,signingKey,now}={}){
  ...
  const file=path.join(directory,hash('FAMILY_PRAISE_JOURNAL_V2\n'+family)+'.json');
  const genesis=sign(signingKey,'FAMILY_PRAISE_GENESIS_V2\n'+family);
  const empty=()=>({contract:CONTRACT,family_id:family,checkpoint:genesis,rows:[]});
  const identity=r=>hash('FAMILY_PRAISE_GIFT_V2\n'+family+'\n'+r.giver_member_id+'\n'+r.idempotency_key);
  function normalize(input={}){
    if(!input||typeof input!=='object'||Array.isArray(input))return null;
    if(input.contract!=='TAKY_FAMILY_PRAISE_GIFT_V2'||input.source!=='FAMILY_PRAISE'||
       input.family_id!==family||!clean(input.giver_member_id)||
    ...
    if([input.giver_member_id,input.receiver_child_id,input.idempotency_key].some(
    ...
    if(input.kind==='GEM_GIFT'){
      ...
      return {contract:input.contract,source:input.source,family_id:family,
        giver_member_id:input.giver_member_id,receiver_child_id:input.receiver_child_id,
...
module.exports=Object.freeze({CONTRACT,createFamilyGiftJournal});
